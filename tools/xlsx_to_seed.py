...
The app itself never needs openpyxl; it reads revision_tracker/seed_chapters.json.
...
HEADERS = ["Strand", "Book", "Ch", "Chapter", "Sections", "Level", "Term"]
...
def main(path):
    ...
    assert list(rows[header_idx][:7]) == HEADERS, rows[header_idx]
    chapters = []
    for r in rows[header_idx + 1:]:
        ...
        chapters.append({
            ...
            "terms": [t.strip() for t in r[6].split("&")],
        })
    out = Path(__file__).resolve().parent.parent / "revision_tracker" / "seed_chapters.json"
